...
echo "ansible-playbook -i tests/hosts tests/site.yml -e target=mongo -e docker_privileged=${DOCKER_PRIVILEGED} -e image_name=${DISTRIBUTION}:${DIST_VERSION}_python${PYTHON_VERSION} -e mongodb_package=${package} -e mongodb_version=${MONGODB_VERSION} -e mongodb_replication_replset='rs0' -e mongodb_security_authorization=${mongodb_security_authorization} -e ansible_python_interpreter=${ansible_python_interpreter}"
